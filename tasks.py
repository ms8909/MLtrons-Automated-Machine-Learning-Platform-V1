...
from project.models import Project, MetaType, Meta, DataSet, MergeColumn, TrainingDetail, Metric, TrainingModel, ModelMetric,\
    FeatureImportance, LossVsInteraction, ActualVsPrediction, TestingData, DeployedModel, Graph, ConfusionMatrix, LiftGain, TrainingValidationLoss, DevianceIter, Task
import pusher
...
def send_notification(retry, id, event, message):
    ...
    try:
        ...
        task = Task.objects.filter(project=id).order_by('-created_at').first()
        task.state = simplejson.dumps(message)
        task.save()
...
def read_and_process_csv(dataset_id, dataset_path, user_id, count):
    try:
        ...
        if path:
            ...
            send_notification(0, user_id, 'read_csv', event)
            print("remove path here", dataset_path)
            ...
            send_notification(0, user_id, 'read_csv', event)
            os.remove(dataset_path)
            logger.error('CSV upload failed, path not found')
            # # # # reader.sql.stop()
...
@app.task
def read_and_process_training_csv(testing_id, dataset_path, user_id, count, type=None):
    try:
        reader = save_read(settings.S3_CLIENT_ID, settings.S3_CLIENT_SECRET)
        ...
        if type == 'json':
            frame = reader.read_as_df_json(dataset_path, count)
        else:
            frame = reader.read_as_df_local(dataset_path, count)
...
def start_training(training_id, user_id):
    try:
        ...
        for i in range(max_model):
            ...
            print('ETR', ETR)
            #logger.info('Training ETR: '+ETR+', training_id: '+str(training_id)+ ', user_id:'+str(user_id))
            if ans == True:
                if training.problem_type.name == 'Classification':
                    print("get details")
                    details = t.get_details_classification()
                else:
                ...
                with transaction.atomic():
                    for detail in details:
                        ...
                        if training.problem_type.name == 'Classification':
                            print("without issue here")
                            print("metric name", metric["name"])
                            for count, item in enumerate(metric['name']):
                                print("item")
                                m, created = Metric.objects.get_or_create(name=item, type=Metric.Classification)
                                print ("m", m.name, created)
                                ModelMetric.objects.create(metric=m, model=training_model, value=metric['value'][count])
                        else:
                            for key in metric.keys():
                                m, created = Metric.objects.get_or_create(name = key, type=Metric.NotClassification)
                        ...
                        for key in graphs.keys():
                            ...
                            if key == 'Deviance vs Iter':
                                DevianceIter.objects.create(project=training.project,
                                                            training_model=training_model,
                                                            data=simplejson.dumps(graphs[key], ignore_nan=True))
                            if key == 'lift_gain':
                                LiftGain.objects.create(project=training.project,
                                                            training_model=training_model,
                                                            data=simplejson.dumps(graphs[key], ignore_nan=True))
                            if key == 'confusion_matrix':
                                ConfusionMatrix.objects.create(project=training.project,
                                                            training_model=training_model,
                                                            data=simplejson.dumps(graphs[key], ignore_nan=True))
                            if key == 'Loss vs trees':
                                TrainingValidationLoss.objects.create(project=training.project,
                                                            training_model=training_model,
                                                            data=simplejson.dumps(graphs[key], ignore_nan=True))
...
def start_testing(model_id, user_id):
    try:
        ...
        if frame:
            ...
            send_notification(0, user_id, 'generate_graph_csv', event)
            # # # reader.sql.stop()
            logger.error('Testing failed! frame not found')
            return event
